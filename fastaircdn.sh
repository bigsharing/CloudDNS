...
auth_email="XXXXXX@gmail.com"    #你的CloudFlare注册账户邮箱
auth_key="###########"   #你的CloudFlare账户Global key,位置在域名概述页面点击右下角获取api key （Global key）
zone_name="xxxx.eu.org"     #你的主域名例如 baidu.com
record_name="00011d"    #自动更新的二级域名前缀,例如cloudflare的cdn用00011d，gcore的cdn用00011g，后面是数字，程序会自动添加。二级域名需要已经在域名管理网站配置完成 #二级域名个数，例如配置5个，则域名分别是00011d1、00011d2、00011d3、00011d4、00011d5. 
             
 #后面的信息均不需要修改，让他自动运行就好了。


# 显式设置PATH变量，包括CloudflareST所在的路径 #https://github.com/XIU2/CloudflareSpeedTest 
export PATH="$PATH:/root/orzl_speed/CloudflareST"   ###/root/orzl_speed/CloudflareST ,是ST的绝对路径。
